...
interface LocalReportNotifyEmailProps {
  name: string
  email: string
  phone: string | null
  address: string
...
export default function LocalReportNotifyEmail({
  name,
  email,
  phone,
  address,
  message,
}: LocalReportNotifyEmailProps) {
  const adminUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/admin/lokalrapport-foresporser`

  return (
    <Html>
      <Head />
      <Preview>Ny forespørsel om lokalrapport fra {name}</Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={content}>
            <Text style={heading}>Ny forespørsel om lokalrapport</Text>
            <table style={table}>
              <tbody>
                <tr>
